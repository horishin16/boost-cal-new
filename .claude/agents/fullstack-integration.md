...
```
Step 0: 実装対象の確認（Foundation完了後）
├── ユーザーへのヒアリング
└── detail-plan.md から実装対象を特定
         ↓
Phase 0: 型・スキーマ設計
├── /api-schema-designer
│   ├── ドメインモデル型
...
1. テスト作成（`/repository-test-writer`）
   ```
   /repository-test-writer
   ```

2. 実装（`/repository-implementer`）
   ```
...
1. テスト作成（`/service-test-writer`）
   ```
   /service-test-writer
   ```

2. 実装（`/service-implementer`）
   ```
   /service-implementer
   ```
...
1. テスト作成（`/api-test-writer`）
   ```
   /api-test-writer
   ```

2. 実装（`/api-implementer`）
   ```
   /api-implementer
   ```
...
1. テスト作成（`/component-test-writer`）
   ```
   /component-test-writer
   ```

2. 実装（`/component-builder`）
   ```
   /component-builder
   ```
...
統合テスト・動作確認エージェント（`/integration-checker`）を呼び出す：
...
| エージェント | 役割 |
|------------|------|
| **api-schema-designer** | 型・スキーマ設計（models, schemas） |
| **repository-test-writer** | Repository テスト作成（RED） |
| **repository-implementer** | Repository 実装（GREEN） |
| **service-test-writer** | Service テスト作成（RED） |
| **service-implementer** | Service 実装（GREEN） |
| **api-test-writer** | API/Server Actions テスト作成（RED） |
| **api-implementer** | API/Server Actions 実装（GREEN） |
| **component-test-writer** | UI コンポーネントテスト作成（RED） |
| **component-builder** | UI コンポーネント実装（GREEN） |
| **integration-checker** | E2E・統合テスト・動作確認 |
